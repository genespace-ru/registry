...
public class WDLHandler implements LanguageHandlerInterface {
    ...
    public static void checkForRecursiveLocalImports(String content, Set<SourceFile> sourceFiles, Set<String> absolutePaths, String parent) throws ParseException
    {
        ...
        for ( String line : lines )
        {
            ...
            while ( m.find() )
            {
                ...
                { // Don't resolve URLs
                    ...
                    Optional<SourceFile> sourcefile = sourceFiles.stream().filter( sourceFile -> sourceFile.getAbsolutePath().equals( absolutePath ) ).findFirst();
     ...
     */
    public Optional<String> reportValidationForLocalRecursiveImports(String primaryDescriptorContent, Set<SourceFile> sourcefiles, String primaryDescriptorFilePath)
    {
        try
        {
    ...
    public VersionTypeValidation validateEntrySet(Set<SourceFile> sourcefiles, String primaryDescriptorFilePath, String type)
    {
        ...
        if( filteredSourceFiles.size() > 0 )
        {
            try
            {
                Optional<SourceFile> primaryDescriptor = filteredSourceFiles.stream().filter( sourceFile1 -> Objects.equals( sourceFile1.getPath(), primaryDescriptorFilePath ) )
                        .findFirst();
                ...
                if( primaryDescriptor.isPresent() )
                {
                    if( primaryDescriptor.get().getContent() == null || primaryDescriptor.get().getContent().trim().replaceAll( "\n", "" ).isEmpty() )
                    {
                        validationMessageObject.put( primaryDescriptorFilePath,
                                "The primary descriptor '" + primaryDescriptorFilePath + "' has no content. Please make it a valid WDL document if you want to save." );
                ...
                for ( SourceFile sourceFile : filteredSourceFiles )
                {
                    if( !Objects.equals( sourceFile.getPath(), primaryDescriptorFilePath ) && sourceFile.getContent() != null )
                    {
                        if( sourceFile.getContent().trim().replaceAll( "\n", "" ).isEmpty() )
                        {
                            ...
                            {
                                validationMessageObject.put( primaryDescriptorFilePath,
                                        "File '" + sourceFile.getPath() + "' has no content. Either delete the file or make it a valid WDL document." );
                            }
                            else if( Objects.equals( sourceFile.getType(), DescriptorLanguage.FileType.WDL_TEST_JSON ) )
                            {
                                validationMessageObject.put( primaryDescriptorFilePath,
                                        "File '" + sourceFile.getPath() + "' has no content. Either delete the file or make it a valid WDL JSON/YAML file." );
                            }
                            else
                            {
                                validationMessageObject.put( primaryDescriptorFilePath,
                                        "File '" + sourceFile.getPath() + "' has no content. Either delete the file or make it valid." );
                            }
                            return new VersionTypeValidation( false, validationMessageObject );
                        }
                ...
                catch (IOException | CustomLoggedException e)
                {
        ...
        for ( String line : lines )
        {
            ...
            while ( m.find() )
            {
                ...
                { // Don't resolve URLs
                    ...
                    {
                        throw new CustomLoggedException( ERROR_PARSING_WORKFLOW_YOU_MAY_HAVE_A_RECURSIVE_IMPORT );
                    }
                    else
                    {
                        URL url = new URL( match );
                        try (InputStream is = url.openStream(); BoundedInputStream boundedInputStream = new BoundedInputStream( is, FileUtils.ONE_MB ))
                        {
    ...
    @Override
    public Map<String, SourceFile> processImports(String repositoryId, String content, WorkflowVersion version, GitHubRepository sourceCodeRepoInterface, String filepath)
    {
        return processImports( repositoryId, content, version, sourceCodeRepoInterface, new HashMap<>(), filepath );
    }

    private Map<String, SourceFile> processImports(String repositoryId, String content, WorkflowVersion version,
                GitHubRepository sourceCodeRepoInterface, Map<String, SourceFile> imports, String currentFilePath) {
        ...
        for ( String importPath : currentFileImports )
        {
            ...
            if( !imports.containsKey( absoluteImportPath ) )
            {
                ...
                {
                    GitHubRepository.LOG.error( "Could not read: " + absoluteImportPath );
                    continue;
                }
                SourceFile importFile = SourceFile.limitedBuilder().type( DescriptorLanguage.FileType.DOCKSTORE_WDL ).content( fileResponse ).path( importPath )
                        .absolutePath( absoluteImportPath ).build();
    ...
    /**
     * Check if the input semantic version string is greater than the newest currently supported WDL semantic version
     * string
     * 
